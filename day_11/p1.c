...
int main() {
    ...
    for (i = 0; i < gindex; ++i) for (j = i + 1; j < gindex; ++j) {
        ...
        // add extra step for empty rows
        for (k = imin(gd[i], gd[j]), end = imax(gd[i], gd[j]); k < end; ++k) if (!row[k]) ++total;
        // add extra step for empty cols
        for (k = imin(gx[i], gx[j]), end = imax(gx[i], gx[j]); k < end; ++k) if (!col[k]) ++total;
    }
    printf("total %ld\n", total);
    fclose(f);
}
