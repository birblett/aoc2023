...
int to_index['T' + 1] = {['A'] 12, ['K'] 11, ['Q'] 10, ['T'] 9, ['9'] 8, ['8'] 7, ['7'] 6, ['6'] 5, ['5'] 4, ['4'] 3, ['3'] 2, ['2'] 1, ['J'] 0};

// calculate relative card strength
int get_score(char* s) {
    int counts[13] = {0}, i, max = 0, max2 = 0, tmp;
    for (i = 0; i < 5; ++i) counts[to_index[s[i]]]++;
    // exclude jack from calculating card maxes
    for (i = 1; i < 13; ++i)
        if ((tmp = counts[i]) > max && ((max2 = max) + 1)) max = tmp;
        else if (tmp > max2) max2 = tmp;
    // add num jacks to max while calcing final strength
    return (max = max + counts[0]) - ((max < 4 && max2 != 2) ? 1 : 0) + (max < 3 ? 0 : 1);
}
